...
# WebAppsを東西リージョンに作成
webAppId1=`az webapp create --resource-group $rgName --name $webAppName1 --plan $appServicePlanName1 --query id --output tsv`
webAppId2=`az webapp create --resource-group $rgName --name $webAppName2 --plan $appServicePlanName2 --query id --output tsv`
